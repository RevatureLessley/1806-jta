	...
	xhr.onreadystatechange = function()
	{
		if(xhr.readyState==4)
		{
			...
			for(index in data)
			{
				...
				let td1t = document.createTextNode(data[index]["reimbursement_id"]);
				let td2t = document.createTextNode(data[index]["event_id"]);
				let td3t = document.createTextNode(data[index]["amnt_req"]);
				let td3t = document.createTextNode(data[index]["bc_approve"]);
